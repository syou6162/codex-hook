...
///
/// Legacy codex-hook output used by the existing PreToolUse/UserPromptSubmit
/// processing paths.
#[derive(Serialize)]
...
/// Current Codex universal hook output field for user-visible warnings.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SystemMessage<'a> {
    pub(crate) system_message: &'a str,
}

/// Current Codex PostToolUse blocking output.
#[derive(Serialize)]
pub(crate) struct PostToolUseBlock<'a> {
...
/// Build a Codex universal hook output JSON string with `systemMessage`.
pub(crate) fn build_system_message_json(message: &str) -> String {
    serde_json::to_string(&SystemMessage {
        system_message: message,
    })
    .expect("JSON serialization should not fail")
}

/// Build a Codex PostToolUse output JSON string that blocks with `reason`.
pub(crate) fn build_post_tool_use_block_json(reason: &str) -> String {
    serde_json::to_string(&PostToolUseBlock {
        decision: "block",
        reason,
    })
    .expect("JSON serialization should not fail")
